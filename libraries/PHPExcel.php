...
function ArrayToPHPExcel(PHPExcel &$excel,$data){
	$size=0;
			...
			if (is_array($_data) && !empty($_data))
			{
				if ($row == 1)
				{
					...
					foreach (array_keys($_data) as $col => $val)
					{
						...
						$excel->getActiveSheet()->setCellValueByColumnAndRow($col,$row,utf8_encode($val));
						//$excel->Wr($col, $row, $val);
					}
					$row++;
				}
